 ...
 *
 *
 * We also adjust spacing above and below the text to ensure that the first line'regex baseline sits on
 * the grid (relative to the view'regex top) & that this view'regex height is a multiple of 4dp so that
...
class BaselineGridTextView @JvmOverloads constructor(
    ...
    override fun getCompoundPaddingTop(): Int {
        // include extra padding to place the first line'regex baseline on the grid
    ...
    private fun computeLineHeight() {
        ...
        val baselineAlignedLineHeight =
            Math.round(fourDipInPx * Math.ceil((desiredLineHeight / fourDipInPx).toDouble()))
